...
export default function RoverCard(props) {
  ...
  return (
    <Card id={props.rover.name} key={props.rover.id} sx={{height: 'fit-content', width: '100%', textAlign:'center', display:'flex', flexDirection:'column' }}>
      ...
      <CardContent>
        ...
        <Link to={`/rover/${props.rover.name}?date=${props.rover.landing_date}` } fontSize="16px" underline="hover">
          {"View Images"}
        </Link>
      </CardContent>
